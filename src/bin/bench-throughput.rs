...
impl TemplateSet {
    fn get_templates() -> Vec<(&'static str, &'static str)> {
        vec![
            ...
            ("Lower", "{lower}"),
            ("Reverse", "{reverse}"),
            ("Replace extension", "{replace:s/\\.txt$/.md/}"),
            ("Replace normalize slashes", "{replace:s/\\/\\/+/\\//g}"),
            ("Regex extract filename", "{regex_extract:[^/]+$}"),
            ("Strip ANSI", "{strip_ansi}"),
            ("Trim", "{trim}"),
            ("Pad right", "{pad:80: :right}"),
            ("Pad left", "{pad:80:_:left}"),

            // ===== Split Operations =====
            ("Split all components", "{split:/:..}"),
            ("Split filename", "{split:/:-1}"),
            ("Split directory", "{split:/:0..-1}"),
            ("Split first 3 components", "{split:/:0..3}"),

            // ===== List Operations on path components =====
            ("Join components", "{split:/:..|join:/}"),
            ("Filter lowercase dirs", "{split:/:..|filter:^[a-z]+$}"),
            ("Filter not hidden", "{split:/:..|filter_not:^\\.}"),
            ("Sort components", "{split:/:..|sort}"),
            ("Sort desc", "{split:/:..|sort:desc}"),
            ("Unique components", "{split:/:..|unique}"),
            ("Slice middle components", "{split:/:..|slice:2..5}"),
            ("Slice last 3", "{split:/:..|slice:-3..}"),
            ("Map upper components", "{split:/:..|map:{upper}}"),
            ("Map trim components", "{split:/:..|map:{trim}}"),
            ...
            ("Uppercase all components", "{split:/:..|map:{upper}|join:/}"),
            ("Normalize and lowercase", "{replace:s/\\/\\/+/\\//g|lower}"),
